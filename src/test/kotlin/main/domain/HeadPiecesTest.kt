...
class HeadPiecesTest {
    ...
    @Test
    fun empty() {
        val headMinos = listOf<MinimalOperationWithKey>()
        val headPieces = HeadPieces(headMinos, Piece.T)
        assertThat(headPieces.representation).isEqualTo("T")
        assertThat(headPieces.allPieceCounter).isEqualTo(PieceCounter(listOf(Piece.T)))
    }
}